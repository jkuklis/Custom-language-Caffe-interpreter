...
instance Show MType where
    show (MType t) = show t
    show (UType t) = show t
    show NotYet = "unttyped"
    show (MBreak t) = show t
    show (MCont t) = show t
...
stronger (MBreak t1) t2 = t2
stronger t1 (MBreak t2) = t1
stronger (MCont t1) t2 = t2
stronger t1 (MCont t2) = t1
...
incrCounter pos = modify $ M.insert 0 $ RInteger $ pos + 1
incrCounterT pos =  modify $ M.insert 0 $ Counter $ pos + 1
    ...
    show (RStruct struct m) = 
        if M.empty == m
            then struct
                ...
                let repr = concat (fmap (\(x,y) -> x ++ " = " ++ (show y) ++ ", ") (M.toList m))
                in struct ++ " {" ++ ((reverse . (drop 2) . reverse) repr) ++ "}"
    show RBreak = "break"
    show RCont = "cont"
...
eval expr = case expr of
    ...
    ETab t size 
            -> let retTab = \s x -> return $ RTab s $ replicate (fromIntegral s) x
                in case size of
                    (s:[]) -> retTab s $ defVal t
                    (s:ss) -> do
                       lr <- eval (ETab t ss)
    ...
    StructIni (Ident s) es 
            -> case es of
                ...
                ((AtrInR (Ident ss) e):es') -> do
                    re <- eval e
                    RStruct struct m <- eval $ StructIni (Ident s) es'
                    return $ RStruct s $ M.insert ss re m
            ...
            -> do
                ...
                let 
                    ...
                    modMap mm (Ident as : ids) = do
                        ...
                        let (am', fin) = modMap mm ids
                        (M.insert as (RStruct struct am') mm, fin)

                let (m', fin) = modMap m ids
...
runStmt toRun@(stmt:stmts) =
    case stmt of
        ...
        Break               
            -> return $ RReturn RBreak
        
        Cont                
            -> return $ RReturn RCont
...
evalT expr = case expr of
    ...
    Not e               
            -> do
                ...
                case te of
                    ...
                    _ -> throwError $ "Wrong type for 'not' operation: " ++ (show te)
    ...
    Neg e               
            -> do
                ...
                case te of
                    MType Int -> return te
                    _ -> throwError $ "Wrong type for integer negation: " ++ (show te)
    ...
    ItoS e              
            -> do
                te <- evalT e
                case te of
                    MType Int -> return $ MType Str
                    _ -> throwError $ "Wrong type for int to string conversion: " ++ (show te)
    ...
    EAdd e1 op e2       
            -> do
                ...
                if t1 == MType Int && (t2 == MType Int)
                    ...
                    else throwError $ "Wrong type for addition (at least 1): " ++ (show t1) ++ " or " ++ (show t2)
    ...
    EMul e1 op e2       
            -> do
                ...
                if t1 == MType Int && (t2 == MType Int)
                    then return $ MType Int
                    else throwError $ "Wrong type for multiplication (at least 1): " ++ (show t1) ++ " or " ++ (show t2)
    ...
    ERel e1 op e2       
            -> do
                ...
                if t1 == MType Int && (t2 == MType Int)
                    ...
                    else throwError $ "Wrong type for comparison (at least 1): " ++ (show t1) ++ " or " ++ (show t2)
    ...
    EAnd e1 e2          
            -> do
                ...
                if t1 == MType Bool && (t2 == MType Bool)
                    then return $ MType Bool
                    else throwError $ "Wrong type for 'and' operation (at least 1): " ++ (show t1) ++ " or " ++ (show t2)
    ...
    EOr e1 e2          
            -> do
                ...
                if t1 == MType Bool && (t2 == MType Bool)
                    then return $ MType Bool
                    else throwError $ "Wrong type for 'or' operation (at least 1): " ++ (show t1) ++ " or " ++ (show t2)
            ...
            -> let helper t l = case l of
                    ...
                    (e:[]) -> do
                        ...
                        if te /= NotYet && (eq t te)
                            ...
                            else throwError $ "Wrong type in table initialization: " ++ (show te) ++ " instead of " ++ (show t)
    ...
    StructIni (Ident stru) es 
            -> do
                ...
                case str of
                        ...
                        let 
                            f [] already = 
                                if length already == (M.size sm) 
                                    then Right ""
                                    else Left $ "Attributes list in struct initialization too short for: " ++ stru

                            f ((AtrInR (Ident s) e):es') alr =
                                if elem s alr
                                    then Left $ "Attribute occurs twice in struct initialization: " ++ s ++ " for " ++ stru
                                    else do
                    ...
                    _ -> throwError $ "No such struct type for struct initialization: " ++ stru
    ...
    ETabIndex (Ident s) l 
            -> do
                let 
                    ...
                    getTypeFromTab _ (lh:lt) = Left $ "Too deep into tab (not enough dimensions): " ++ s
                            ...
                            Just (MType (Fun t targs)) -> do 
                                let
                                    ...
                                    place (t:ta) (e:es) = do
                                        ...
                                        else throwError $ "Wrong function argument type: " ++ (show re) ++ " instead of " ++ (show t) ++ " in function " ++ s
                                ...
                                else throwError $ "Wrong number of arguments: " ++ (show (length targs)) ++ " for function " ++ s

                            Just t -> throwError $ "Wrong type: " ++ (show t) ++ " instead of function type for identifier " ++ s

                    _ -> throwError $ "Undefined function: " ++ s
    ...
    ELambda t params st 
            ...
            in do
                ...
                if (res == MType t) || (t == Void && res == NotYet) 
                    ...
                    else throwError $ "Wrong return type in anonymous function: " ++ (show res) ++ " instead of " ++ (show t)
    ...
    ChangeAttr attr op e 
            -> do
                te <- evalT e
                case te of
                    MType Int -> evalT (IncrDecrA attr Incr)
                    _ -> throwError $ "Not an int (trying to change numerical attribute): " ++ (show te)
            ...
            -> do
                ...
                case pos of
                    Just (VDef p) -> do
                        org <- gets $ M.lookup p
                        case org of
                            Just (MType Int) -> return $ MType Int
            ...
            case val of
                MType Bool -> do
                    ...
                    if eq ret val1 && (eq ret val2)
                        ...
                        else throwError $ "Different return types in if-else cond: " ++ (show val1) ++ " or " ++ (show val2) ++ " instead of " ++ (show ret)

                _ -> throwError $ "Wrong type in ifelse condition: " ++ (show val)
                ...
                then throwError $ "Wrong type of main: " ++ (show t)
                    ...
                    in do
                        ...
                        if (res == MType t) || (t == Void && res == NotYet) 
                            ...
                            else if (s == "main") && (res == NotYet) then throwError "No return in main"
                            else throwError $ "Wrong return type: " ++ (show res) ++ " instead of " ++ (show t) ++ " in " ++ s
...
tryRun s = do
    let ts = myLexer s in case pProg ts of
        Bad s -> do 
